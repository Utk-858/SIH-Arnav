...
class PDFService {
    ...
    static async generateDietChartPDF(dietPlan, patient) {
    ...
    static async getFoodItemsForDietPlan(dietPlan) {
        ...
        if (foodNames.length > 0) {
            try {
                const foods = await firestore_1.FirestoreService.getAll('foods', (query) => query.where('name', 'in', foodNames.slice(0, 10)) // Firestore 'in' limit is 10
                );
                foods.forEach(food => {
    ...
    static calculateNutritionalSummary(dietPlan, foodItems) {
        let totalCalories = 0;
        let totalProtein = 0;
        let totalCarbs = 0;
        let totalFat = 0;
    ...
    static generateDietChartHTML(dietPlan, patient, nutritionalSummary, foodItems) {
        return `
<!DOCTYPE html>
<html lang="en">
<head>
