...
import { notificationsService, notificationSettingsService } from './notifications';
import { patientsService } from './firestore';
import { dietPlansService } from './dietPlans';
...
function getTomorrowAtTime(timeString: string): Date {
...
export async function sendDueNotifications() {
  try {
    const dueNotifications = await notificationsService.getDueNotifications();
...
export async function scheduleDailyRemindersForAllUsers() {
  try {
